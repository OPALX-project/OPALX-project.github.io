...
A good example for a coding-style is the https://www.kernel.org/doc/html/v4.10/process/coding-style.html[Linux coding-style]. It's worth to read.
...
C{pp}11 braced list (see example below).
...
* http://astyle.sourceforge.net[Artistic Style] is a source code indenter, formatter, and beautifier for the C, C{pp}, C# and Java programming languages.
* http://uncrustify.sourceforge.net/[Uncrustify] a highly configurable, easily modifiable source code beautifier.
* http://universalindent.sourceforge.net/[UniversalIndentGUI] offers a live preview for setting the parameters of nearly any indenter. You change the value of a parameter and directly see how your reformatted code will look like. Save your beauty looking code or create an anywhere usable batch/shell script to reformat whole directories or just one file even out of the editor of your choice that supports external tool calls"
...
* Use `std::unique_ptr`/`std::shared_ptr` and `std::make_unique` (once https://gitlab.psi.ch/OPAL/src/-/issues/607[Increase C{pp} Standard to c{pp}17] is implemented) / `std::make_shared` where ever possible.
...
http://www.yolinux.com/TUTORIALS/LinuxTutorialC++CodingStyle.html#DOXYGEN[http://www.yolinux.com/TUTORIALS/LinuxTutorialC{pp}CodingStyle.html#DOXYGEN]

https://google.github.io/styleguide/cppguide.html[Google C{pp} Style Guide]
