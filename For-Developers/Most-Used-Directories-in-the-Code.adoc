...
Within 6 steps a new element is added to OPALX. Six new files are created **Ip**.{h,cpp}, **IpRep**.{h,cpp}
...
1. ‎src/AbsBeamline: CMakeLists.txt, BeamlineVisitor.{h,cpp}, **Ip**.{h,cpp}, ElementBase.{h,cpp}, SpecificElementVisitor.h‎

2. src/Alogorithms: DefaultVisitor.{h,cpp}, ParallelTracker.{h,cpp}, IndexMap.cpp

3. src/BasciActions: Option.cpp

4. src/BeamlineCore: CMakeLists.txt, **IpRep**.{h,cpp}

5. src/Elements: CMakeLists.txt, **OpalIp**.{h,cpp}

6. src/OpalConfigure: Configure.cpp
