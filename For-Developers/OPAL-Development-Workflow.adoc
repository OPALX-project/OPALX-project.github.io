...
* file all changes (bugfixes and feature changes) in the https://gitlab.psi.ch/OPAL/src/issues[issue tracker] of Gitlab
...
If your merge request includes a significant number of commits, please consider squashing the commits before merging, in order to simplify the commit history of the branch. A very simple way to do it is using a https://gitlab.psi.ch/help/user/project/merge_requests/squash_and_merge[tool] integrated in GitLab
...
. Open an issue in the https://gitlab.psi.ch/OPAL/src[OPAL project]
. Create a fork on Gitlab.
. Create a new branch in your fork on Gitlab. This is the branch you have to work on. Do _not_ work on the master-branch!
...
. Create a merge request in the https://gitlab.psi.ch/OPAL/src[_OPAL_ project], select at least two approvers and resolve work in progress (WIP) status.
...
. Open the https://gitlab.psi.ch/OPAL/src[OPAL project] and push the fork button
...
. Change into the cloned repository and add the upstream repository as a _remote_
+
----
...
. Open a new merge request in the https://gitlab.psi.ch/OPAL/src[_OPAL_ project].
