= Release Notes for https://gitlab.psi.ch/OPAL/src/-/milestones/30[OPAL 2021.1]
:pp: {plus}{plus}
...
* New Undulator element with its own FDTD electromagnetic solver, making use of the https://github.com/aryafallahi/mithra[(MITHRA)] library (#535)
...
* Since link:ReleaseNotes/OPAL-2.4[version 2.4] the `DISTRIBUTION` momentum input is changed to momentum (instead of energy). To avoid confusion, this is also made clear in the `DISTRIBUTION` command, `INPUTMOUNITS = EV` is changed to `INPUTMOUNITS = EVOVERC`  (#649)
