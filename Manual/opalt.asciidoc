...
system is denoted by latexmath:[(X, Y, Z) \in K]. In Figure <<KS1,1>> of
...
Figure <<KS1,1>>. The local reference system
...
.Figure 2 : Illustration of latexmath:[K_\text{sc}] and latexmath:[K_s]
image:figures/opalt/curvcoords.png[]

Both coordinate systems are described in Figure <<KS2,2>>.
...
auto-phases the rf-structure, see Appendix link:autophase[Auto-phasing Algorithm]. The multi map
...
the example given in Figure <<KS1,1>>. An implicit drift is modelled as an
...
[#OPALTSchemeSimple]
.Figure 3 : Schematic workflow of _OPAL-t_’s execute method.
image:figures/opalt/flowdiagram.png[]

A regular time step in _OPAL-t_ is sketched in
Figure <<OPALTSchemeSimple,3>>. In order to compute the coordinate system
...
As can be seen from Figure <<OPALTSchemeSimple,3>> the integration of the
...
* `--background r g b` to specify background color of web canvas
...
* [[[bib:qiang2006-1]]] J. Qiang et al., _Three-dimensional quasi-static model for high brightness beam dynamics simulation_, Phys. Rev. ST Accel. Beams 9 (2006).
* [[[bib:qiang2006-2]]] J. Qiang et al., _Erratum: three-dimensional quasi-static model for high brightness beam dynamics simulation_, Phys. Rev. ST Accel. Beams 10, 12990 (2007).
