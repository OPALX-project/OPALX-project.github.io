...
_OPAL-t_ is a super-set of _Impact-t_ and can be used to model guns,
...
every night. The input file is found in Section https://gitlab.psi.ch/OPAL/src/wikis/Manual/tutorial#sec:examplesbeamlines[Examples of Particle Accelerators and Beamlines].
...
statement see Section https://gitlab.psi.ch/OPAL/src/wikis/Manual/control#sec:option[Option Statement], with the flag `PSDUMPFREQ`. The file is
...
see Section https://gitlab.psi.ch/OPAL/src/wikis/Manual/control#sec:option[Option Statement] with the flag `STATDUMPFREQ`.
...
See Appendix <<changelog>> for a detailed list of changes in _OPAL_.
