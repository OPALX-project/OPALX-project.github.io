...
concatenated using the `&` operator see Table <<stroperator,1>>. An operand
in a string can also use the function `STRING` see Table <<stringfun,2>>.
...
see Table <<logoperator,3>> and logical operands:
...
A real expression is built from operators see Table <<operator,4>> and
...
An expression can be formed using operators see Table <<operator,4>> and
functions see Table <<realfun,5>> acting on operands see Section <<sec:operand>>.
...
see Table <<constant,7>>. Their names must not be used for user-defined
