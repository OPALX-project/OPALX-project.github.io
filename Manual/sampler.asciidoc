...
// Sampling methods
SM1: SAMPLING, VARIABLE="nstep", TYPE="FROMFILE",    FNAME="nsteps.dat";
...
QUIT;
....
The samples of ```nstep``` are provided by ```nsteps.dat``` that could look like as follows:
....
MX    nstep
16    10
17    11
...
25    19
....
Although the file contains samples for ```MX```, too. They are not considered.
