...
PSDUMPFRAME::
  ...
  reference particle, yielding unexpected results.
...
SCAN::
  ...
  Section link:track#sec:randmach[Track a Random Machine]. ````Warning: files are not
necessarily closed when a new scan begins```` We recommend using runOPAL.py (https://gitlab.psi.ch/OPAL/src/wikis/runOPAL)
...
ASCIIDUMP::
  ...
  elements: Probe, Collimator, Monitor, Stripper and global
  losses.
NLHS::
...
constants see Table link:format#constant[Predefined Symbolic Constants].
