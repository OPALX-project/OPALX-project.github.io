...
Before starting to track, a beam line see Chapter link:lines[Beam Lines] and a beam
see Chapter link:beam[Beam Command] must be selected. The time step (`DT`) and the
...
LINE::
  The label of a preceding `LINE` see Chapter link:lines[Beam Lines] (no default).
BEAM::
