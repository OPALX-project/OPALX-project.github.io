...
The directory `cain/` in this note contains a reproducible helper script that
clones CAIN, applies the small macOS / `gfortran` portability patch set, and
compiles a local binary.

From `gamma-gamma/cain`, a fresh clone and build is
...
To recompile an existing local source tree without recloning, use
...
The same workflow is also available through a small CMake wrapper:
...
The script applies the following source-level portability fixes before
compilation:

- `src/evufnchar.f`: change `WRITE(TEXT,'(I)')` to `WRITE(TEXT,'(I0)')`
- `src/transport.f`: change `WRITE(MAGNAM,'(I)')` to `WRITE(MAGNAM,'(I0)')`
- `src/lsrrdfl.f`: replace the old end-of-line marker definition with
