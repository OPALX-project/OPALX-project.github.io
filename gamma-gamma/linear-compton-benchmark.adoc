...
forward electron direction latexmath:[+\hat z].
...
- the speed of light latexmath:[c] and the reduced Planck constant latexmath:[\hbar] are
...
The current implementation is intentionally narrow but now includes a dedicated
physics helper in `src/Physics/LinearCompton.*`:
...
- unit tests verify the Thomson limit, exact recoil endpoints, spectrum
  normalization, the exact 90 degree forward-scattering benchmark, and a
  weak-field CAIN spectrum comparison
...
The next useful validation and implementation steps are

- add polarized linear Compton support
- promote the single-electron spectrum helper to photon sampling for future
  photon-bunch population
- separate the weak-field benchmark from future nonlinear laser cases
...
For the present macOS / `gfortran` build, three small portability fixes were
needed in the CAIN source before compilation:

- `src/evufnchar.f`: change `WRITE(TEXT,'(I)')` to `WRITE(TEXT,'(I0)')`
- `src/transport.f`: change `WRITE(MAGNAM,'(I)')` to `WRITE(MAGNAM,'(I0)')`
- `src/lsrrdfl.f`: replace the old BOZ character initialization of end-of-line
  markers with
+
[source,fortran]
----
...
After those edits, CAIN builds locally with

[source,sh]
----
mkdir -p /tmp/CAIN-build
cd /tmp/CAIN-build

gfortran -c -O2 -std=legacy -ffixed-line-length-none \
  -fallow-argument-mismatch \
  -I/tmp/CAIN-src/src/include -J/tmp/CAIN-build \
  /tmp/CAIN-src/src/module/flchtyp.f \
  /tmp/CAIN-src/src/module/arraymod.f \
  /tmp/CAIN-src/src/module/bbcom.f \
  /tmp/CAIN-src/src/module/bbpkcm.f \
  /tmp/CAIN-src/src/module/beamcm.f \
  /tmp/CAIN-src/src/module/lasrdata.f \
  /tmp/CAIN-src/src/module/lumcom.f \
  /tmp/CAIN-src/src/module/nlcplptab.f \
  /tmp/CAIN-src/src/module/beamln.f \
  /tmp/CAIN-src/src/module/evlmod.f \
  /tmp/CAIN-src/src/module/matchmod.f \
  /tmp/CAIN-src/src/module/readmod.f

gfortran -c -O2 -std=legacy -ffixed-line-length-none \
  -fallow-argument-mismatch \
  -I/tmp/CAIN-src/src/include -I/tmp/CAIN-build -J/tmp/CAIN-build \
  /tmp/CAIN-src/src/*.f /tmp/CAIN-src/src/unix/*.f
...
=== CAIN Weak-Field Reference Deck

The file `cain-linear-compton-90deg.i` is now the weak-field reference deck
used for the first OPALX spectrum comparison.

Its benchmark parameters are
...
- resulting CAIN photon yield: about `1.04e4` photons
...
The stored weak-field histogram references in this directory are
...
These are normalized density histograms with columns
...
.Generated weak-field CAIN vs OPALX spectrum overlay
image::reference-data/linear-compton-90deg-xi029-comparison.png[width=85%]

=== CAIN Beam File Format
...
=== 90 Degree Crossing in the Electron Rest Frame

For the present benchmark the laboratory crossing angle is latexmath:[90^\circ]. In
that case the incoming laser photon energy in the electron rest frame is
...
because latexmath:[\cos \alpha = 0] in
...
The incoming laser direction in the electron rest frame is not collinear with
the electron boost axis. If the outgoing photon is required to be forward in the
laboratory frame, then its direction in the electron rest frame is along
latexmath:[+\hat z], while the angle between the incoming and outgoing photons is
...
which is algebraically equivalent to the stable exact laboratory-frame formula
used in the OPALX unit test,
