...
'''
=== Building Boost
The installation of OPAL fails with the currently provided Boost module (```Boost/1.65.0-CrayGNU-17.08-python3```). Thus, we install it locally. Perform the following steps:
...
> module load daint-mc
> module switch PrgEnv-cray/6.0.4 PrgEnv-gnu/6.0.4
> ./bootstrap.sh --prefix=$HOME/boost_1_65_1/build
> ./b2 install
```

Finally, we set some evironment variables:
```
> export BOOST_HOME=$HOME/boost_1_65_1/build/
