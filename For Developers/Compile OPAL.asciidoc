...
Optional, if you want to compile with CUDA support:
```
...
```
module use /afs/psi.ch/project/amas/modulefiles
module load opal-toolschain/2.1
```
...
sudo port install boost +${GCC} +openmpi
sudo port install hdf5-18 +${GCC} +cxx +openmpi +threadsafe 
