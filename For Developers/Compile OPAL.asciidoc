...
2. We observed some issues compiling GCC 7.3.0 on macOS 10.13 with Xcode 10. First issue was that `bits/basic_ios.h` was not found, this might have been an issue due too many parallel make processes and could be solved by running make with a single process. Second issue was, that `g++` has called instead of the new build `xg++`. 
