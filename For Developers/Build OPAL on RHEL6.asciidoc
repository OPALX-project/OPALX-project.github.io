...
Unfortunately most of the software to build OPAL is not available as RPM from the standard repositories including EPEL. The following packages must be build and installed:
...
curl -L \
	...
	"https://gmplib.org/download/$P/$P-$V.tar.bz2"
```
+
1. **Unpack GMP**
+
...
curl -L \
	--output "${DOWNLOADS_DIR}/$P-$V.tar.bz2" \
	"https://support.hdfgroup.org/ftp/HDF5/current${V_MAJOR}${V_MINOR}/src/hdf5-$V.tar.bz2"
```
+
1. **Unpack hdf5**
+
...
Anyway you can try with other targets. For autodetection just run `make`.
+
```
make TARGET=CORE2
...
CC=mpicc CXX=mpicxx cmake \
        ...
        -D TPL_METIS_LIBRARIES:PATH="${PREFIX}/lib" \
        -D TPL_ParMETIS_INCLUDE_DIRS:PATH="${PREFIX}/include" \
        -D TPL_ParMETIS_LIBRARIES:PATH="${PREFIX}/lib" \
