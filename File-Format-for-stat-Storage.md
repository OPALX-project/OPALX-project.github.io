...
```
where 
* `name_i` (1 <= i <= I+J) are the names of the design variables / objectives / columns from the stat file. The type of the `name_i` is string with max length 64 (should we sufficient). 
* `I` is the sum of the number of design variables and objectives (exact number can't be specified since both the design variable and the objectives are the users choice)
* `J` is the number of column from the stat file that should be stored (not all columns should be stored, the specific columns are chosen by the users).
