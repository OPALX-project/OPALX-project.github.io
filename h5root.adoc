= H5root is no longer supported! Please use https://gitlab.psi.ch/OPAL/pyOPALTools/wikis/home[pyOPALTools] instead for postprocessing.
:doctype: book
:pp: {plus}{plus}
...
https://www.hdfgroup.org/solutions/hdf5/[HDF5] files written by
https://dav.lbl.gov/archive/Software/H5hut-H5part/[H5hut] (formerly known as H5Part)
via a graphical user interface based on http://root.cern.ch/[ROOT].
https://accelconf.web.cern.ch/accelconf/ICAP2009/papers/thpsc049.pdf[H5root]
...
. as a data browser (no knowledge whatsoever about C{pp} or ROOT is needed)
. as a shared library loaded into an interactive ROOT session (some knowledge of C{pp} and ROOT required).
...
The example macro link:H5root/example.C[example.C] is enclosed in the package.
It was used to produce this jpeg image:

image::H5root/H5rootexample.jpg[TunesClosedOrbitFinder]

== Online Class Documentation

The http://amas.web.psi.ch/tools/H5root/htmldoc/[online class documentation for version 1.2]
is produced with ROOT's http://root.cern.ch/root/html/THtml.html[THtml] class.
